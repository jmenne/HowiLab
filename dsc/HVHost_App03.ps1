...
$session = New-PSSession -VMName App03 -Credential $wcred
Invoke-Command -Session $session -ScriptBlock {
    ...
    Write-Host "LCM auf App03 konfigurieren:" -ForegroundColor Green
    ./01-ConfigureLCM.ps1
    Write-Host "Erstelle MOF Datei für App03:" -ForegroundColor Green
    ./03-ConfigureApp03.ps1

}

# DSC Konfiguration starten
Write-Host "Starte DSC Konfiguration auf App03:" -ForegroundColor Green
...
do {
    ...
    $filesda = Invoke-Command -VMName "App03" -ScriptBlock { (Test-path 'C:\Files\')} -Credential $cred -ErrorAction Ignore
    Write-Host "_-" -NoNewline
    Start-Sleep 4
   }
until (($filesda -eq $True) -and ($testapp.name -eq "App03"))
Write-Host
Write-Host "App03 ist eingerichtet!" -ForegroundColor Yellow
Pause
